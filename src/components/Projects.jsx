...
function ProjectRow({ project, index, onViewAll }) {
  ...
  return (
    <a
      ...
      className="group block border-t border-[#1a1a1a] py-8 hover:bg-white/[0.015] transition-colors duration-200 px-2 -mx-2 animate-fadeInUp"
      style={{ animationDelay: `${index * 0.1}s` }}
    >
      <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-0">
        <span className="font-mono text-[10px] text-[#b0b0b0] md:w-16 shrink-0">{num}</span>

        <div className="md:flex-1">
          <div className="flex items-center gap-3 mb-1">
            <h3 className="font-display text-xl font-bold text-white group-hover:text-[#FF2D78] transition-colors duration-200">
              {title}
            </h3>
            {featured && (
              <span className="font-mono text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#00E5FF]/10 text-[#00E5FF] border border-[#00E5FF]/20 tracking-widest uppercase">
                récent
              </span>
            )}
          </div>
          <p className="text-[#bbb] text-xs font-mono">{type} · {year}</p>
        </div>

        <p className="md:w-[280px] text-[#bbb] text-sm leading-relaxed md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200 shrink-0">
          {desc}
        </p>

        <div className="flex flex-wrap gap-1.5 md:w-[180px] md:justify-end shrink-0">
          {tags.map(t => (
            <span
              key={t}
        ...
        <div className="md:ml-6 shrink-0">
          <ArrowUpRight
            size={16}
            className="text-[#b0b0b0] group-hover:text-[#FF2D78] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-200"
          />
        </div>
      </div>
    </a>
    ...
    <section id="projects" className="relative py-32" style={{ overflowX: 'clip' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 mb-16">
        <div className="flex items-center gap-4">
          <span className="font-mono text-[10px] text-[#b0b0b0] tracking-[4px] uppercase">03 — travaux</span>
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div>
          {featured.map((p, i) => (
            <ProjectRow key={p.num} project={p} index={i} onViewAll={onViewAll} />
          ))}
